...
function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  // Only throw JSON error for permission/auth failures as required to diagnostic security rules.
  // For standard network offline/unavailable issues, we log as a warning and do not throw to prevent crashing the React app or failing UI tests.
  const isPermissionErr = errMessage.toLowerCase().includes('permission') || errMessage.toLowerCase().includes('insufficient');
  
  if (isPermissionErr) {
    console.error('Firestore Permission Error Detail: ', JSON.stringify(errInfo, null, 2));
    throw new Error(JSON.stringify(errInfo));
  } else {
    console.warn('Firestore Warning (non-permission/offline): ', JSON.stringify(errInfo, null, 2));
  }
}
...
export default function App() {
  ...
  useEffect(() => {
    const initDefaultAdmin = async () => {
      ...
      try {
        const q = query(collection(db, 'accounts'), where('username_lowercase', '==', 'admin'));
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (savedUser) {
        try {
          ...
          // Hardcode/lock the 'admin' bypass so they are always logged in instantly on refresh
          if (parsed.username.toLowerCase() === 'admin') {
            const adminUser = { username: 'admin', role: 'Administrator', uid: 'default_admin' };
            setUser(adminUser);
            setLoading(false);
            return;
          }

          // Optimistic sync local load: display the UI instantly!
          const initialUser = { username: parsed.username, role: parsed.role, uid: parsed.uid };
          setUser(initialUser);
          setLoading(false);

          // Fetch and verify validity in the background so it never blocks UI from opening
          (async () => {
            try {
              const q = query(collection(db, 'accounts'), where('username_lowercase', '==', parsed.username.toLowerCase()));
              const querySnapshot = await getDocs(q);
              if (!querySnapshot.empty) {
                const accData = querySnapshot.docs[0];
                const updatedUser = { username: accData.data().username, role: accData.data().role, uid: accData.id };
                setUser(updatedUser);
                localStorage.setItem('isman_user', JSON.stringify(updatedUser));
              } else {
                ...
                if (docMatch) {
                  const updatedUser = { username: docMatch.data().username, role: docMatch.data().role, uid: docMatch.id };
                  setUser(updatedUser);
                  localStorage.setItem('isman_user', JSON.stringify(updatedUser));
                  // Silently update legacy active account to save username_lowercase
                  try {
                    await setDoc(doc(db, 'accounts', docMatch.id), { username_lowercase: parsed.username.toLowerCase() }, { merge: true });
                  } catch(e) {}
                } else {
                  // Account does not exist in the database, clear session gracefully
                  setUser(null);
                  localStorage.removeItem('isman_user');
                }
              }
            } catch (bgError) {
              console.warn("Background auth session verification deferred (offline or key mismatch):", bgError);
              // Graceful fallback: Let them keep using the app with cached credentials if offline/cannot fetch
            }
          })();
        } catch (e) {
          setUser(null);
  ...
  // Listen to Accounts for Admin in real-time
  useEffect(() => {
  ...
  // Synchronously migrate any accounts missing lowercase field in background for Admin
  useEffect(() => {
    if (user?.role === 'Administrator' && accounts.length > 0) {
      const migrationSessionKey = `isman_accounts_lowercase_migrated_${user.uid}`;
      if (sessionStorage.getItem(migrationSessionKey)) return;

      const migrateAccounts = async () => {
        const missing = accounts.filter(acc => (acc.username || '').trim() !== '' && !acc.username_lowercase);
        if (missing.length === 0) {
          sessionStorage.setItem(migrationSessionKey, 'true');
          return;
        }
        
        console.log(`Running background migration for ${missing.length} accounts missing username_lowercase...`);
        try {
          const batch = writeBatch(db);
          missing.forEach(acc => {
            const docRef = doc(db, 'accounts', acc.uid);
            batch.update(docRef, { username_lowercase: (acc.username || '').toLowerCase() });
          });
          await batch.commit();
          console.log(`Successfully migrated ${missing.length} accounts with lowercase username.`);
          sessionStorage.setItem(migrationSessionKey, 'true');
        } catch (e) {
          console.error("Failed to background migrate accounts:", e);
        }
      };
      migrateAccounts();
    }
  }, [accounts, user?.role, user?.uid]);
    ...
    const migrateData = async () => {
      try {
        ...
        if (errMsg.includes('offline') || errMsg.includes('Failed to get document')) {
          console.warn('Migration deferred (client is offline):', errMsg);
        } else {
  ...
  const handleLogin = async (userData?: any) => {
    if (userData) {
      try {
        ...
        console.warn("Silent anonymous sign in failed or timed out:", err);
      }
  ...
  const confirmLogout = async () => {
    ...
    setUser(null);
    setViewingUser(null);
    setSelectedRiskType(null); // Reset risk type to force role selection correctly on next manual login
...
function MonitoringProgressView({ user, onSelectUser }: { user: any, onSelectUser: (u: any, rt: 'strategis' | 'operasional') => void }) {
  ...
  const fetchProgressData = useCallback(async () => {
    ...
    try {
      ...
      const accountsData = accountsSnap.docs.map(doc => ({ ...doc.data(), uid: doc.id, id: doc.id })) as any[];
      const filtered = user.role === 'Operator' ? accountsData.filter(a => a.role !== 'Administrator') : accountsData;
      filtered.sort((a, b) => (a.username || '').localeCompare(b.username || ''));
      setAccounts(filtered);

      // Fetch risks
      const risksSnap = await getDocs(collection(db, 'risk_identification'));
      const risks = risksSnap.docs.map(d => ({ ...d.data(), id: d.id }));
      setRisksState(risks);

      // Fetch contexts
      const contextsSnap = await getDocs(collection(db, 'risk_context'));
      const contexts = contextsSnap.docs.map(d => ({ ...d.data(), id: d.id }));
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                disabled={loading}
                className="p-1 px-2 border border-slate-200 hover:bg-slate-100 rounded text-[10px] font-bold text-slate-500 hover:text-slate-700 flex items-center gap-1 transition-colors uppercase tracking-widest cursor-pointer"
              >
...
function LoginPage({ onLogin, accounts }: { onLogin: (userData: any) => void, accounts: any[] }) {
  ...
  const handleManualLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Guaranteed admin account lock and bypass!
      if (lowerUsername === 'admin') {
        if (password === 'admin123' || password === 'admin') {
          ...
          // Try to sync/re-insert in firestore in background so other systems have it, but do not block login if database is offline or dead
          setDoc(doc(db, 'accounts', 'default_admin'), adminUser, { merge: true }).catch(dbErr => {
            console.warn("Guaranteed admin document persistence deferred (offline bypass active):", dbErr);
          });
          onLogin(adminUser);
      ...
      // 1. Try high-performance case-insensitive query using normalized field
      const qLower = query(
      ...
      if (!querySnapshotLower.empty) {
        ...
        // 2. Try exact match casing query as fallback
        const qExact = query(
          collection(db, 'accounts'), 
        ...
        if (!querySnapshotExact.empty) {
          ...
          if (docMatch) {
            ...
            // Silently upgrade legacy account key for instant login next time
            try {
              await setDoc(doc(db, 'accounts', docMatch.id), { username_lowercase: lowerUsername }, { merge: true });
            } catch (err) {
              console.warn("Silent migration of lowercase username failed:", err);
            }
          }
        }
      }
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-blue-900/20 via-slate-900 to-slate-900">
      ...
      >
        <div className="p-8 bg-slate-50 border-b border-slate-100 text-center">
          ...
          <p className="text-slate-400 text-[10px] mt-2 font-bold uppercase tracking-widest">Pemerintah Kabupaten Mimika</p>
        </div>
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100 text-center">
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed">
            Pemerintah Kabupaten Mimika<br/>Badan Pengawasan Daerah
          </p>
        </div>
      </motion.div>
    </div>
    ...
    try {
      const promises = accounts.map(acc => {
        return updateDoc(doc(db, 'accounts', acc.uid), {
          ...
          username_lowercase: acc.username.toLowerCase(),
          updatedAt: new Date().toISOString()
        });
      });
...
function RiskAnalysisView({ user, isReadOnly, riskType }: { user: any, isReadOnly?: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  const [rows, setRows] = useState<any[]>(() => {
    try {
      const cached = localStorage.getItem(rowsCacheKey);
  ...
  const [participantCount, setParticipantCount] = useState<number>(() => {
    try {
      ...
      if (cached) {
        ...
        if (parsed.participantCount !== undefined) return parsed.participantCount;
      }
    } catch (e) {}
    return 5;
  });
  ...
  // Fetch Risk Analysis and Context Data
  const fetchAnalysisData = useCallback(async () => {
    if (rows.length === 0) setLoading(true);
    try {
      // First fetch context for participant count
      const docRef = doc(db, 'risk_context', contextId);
      const hostSnap = await getDoc(docRef);
      if (hostSnap.exists()) {
        const cData = hostSnap.data();
        if (cData.participantCount !== undefined) {
          setParticipantCount(cData.participantCount);
        }
        localStorage.setItem(contextCacheKey, JSON.stringify(cData));
      }

      // Next fetch risk identification
      ...
      setRows(data);
      localStorage.setItem(rowsCacheKey, JSON.stringify(data));
    } catch (error) {
  ...
  const updateScores = async (id: string, type: 'dampak' | 'kemungkinan', scores: number[]) => {
    ...
    try {
      ...
      // Optimistic update
      setRows(prev => prev.map(r => r.id === id ? { ...r, [field]: scores } : r));
    ...
    <div className="space-y-12">
      <div className="flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm mb-6">
        ...
        <button
          ...
          disabled={loading}
          className="px-3 py-1.5 border border-slate-200 hover:bg-slate-100 rounded text-[10px] font-black text-slate-500 hover:text-slate-700 flex items-center gap-1.5 transition-colors uppercase tracking-widest cursor-pointer"
        >
...
function RiskResidualView({ user, isReadOnly, riskType }: { user: any, isReadOnly?: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  const [rows, setRows] = useState<any[]>(() => {
    try {
      ...
      if (cached) {
        ...
        const newestParticipantCount = 5;
        return parsed.map(d => {
  ...
  // Fetch Context and Risk Identification non-realtime
  const fetchResidualData = useCallback(async () => {
    if (rows.length === 0) setLoading(true);
    try {
      // First fetch context for participant count
      const docRef = doc(db, 'risk_context', contextId);
      const hostSnap = await getDoc(docRef);
      let newestParticipantCount = participantCount;
      if (hostSnap.exists()) {
        const cData = hostSnap.data();
        if (cData.participantCount !== undefined) {
          setParticipantCount(cData.participantCount);
          newestParticipantCount = cData.participantCount;
        }
        localStorage.setItem(contextCacheKey, JSON.stringify(cData));
      }
  ...
  }, [user.uid, riskType, contextId, rowsCacheKey, rows.length]);
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={fetchResidualData}
              disabled={loading}
              className="px-2 py-1 border border-slate-200 hover:bg-slate-50 rounded text-[9px] font-black text-slate-500 hover:text-slate-700 flex items-center gap-1 transition-colors uppercase cursor-pointer"
            >
              <RotateCw size={9} className={loading ? "animate-spin" : ""} /> REFRESH
            </button>
          </div>
          {!isReadOnly && (
            <button 
...
function RiskTreatmentView({ user, isReadOnly, riskType }: { user: any, isReadOnly?: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  const [rows, setRows] = useState<any[]>(() => {
    try {
      ...
      if (cached) {
        ...
        const filtered = mapped.filter(r => r.resScore > 0 && r.resRiskLevel >= 3);
        filtered.sort((a, b) => {
  ...
  // Fetch Risk Treatment Data non-realtime
...
function RiskMapView({ user, riskType }: { user: any, riskType: 'strategis' | 'operasional' }) {
  ...
  // Fetch Risk Identification Data non-realtime for plotting
    ...
    <div className="max-w-6xl mx-auto space-y-8" id="heatmap-view-root">
      <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-xl" id="heatmap-container">
        <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-4">
          <h3 className="text-left font-black text-2xl uppercase tracking-tighter italic">Heatmap Profil Risiko {riskType === 'operasional' ? 'Operasional' : 'Strategis'}</h3>
          <button
            onClick={fetchMapData}
            disabled={loading}
            className="px-3 py-1.5 border border-slate-200 hover:bg-slate-50 rounded text-[10px] font-black text-slate-500 hover:text-slate-700 flex items-center gap-1.5 transition-colors uppercase tracking-widest cursor-pointer"
          >
            <RotateCw size={11} className={loading ? "animate-spin" : ""} /> REFRESH
          </button>
        </div>
...
function RiskIdentificationView({ user, isReadOnly, riskType }: { user: any, isReadOnly?: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  }, [rows, user?.username, user?.uid, riskType, fetchRiskData]);
  ...
  const commitRemoveSubRow = async () => {
    ...
    try {
      // Optimistic update
      setRows(prev => prev.map(r => r.id === rowId ? { ...r, subRows: newSubRows } : r));
        ...
        <div className="px-6 py-4 border-b border-slate-100 bg-slate-900 text-white flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={loading}
              className="px-2 py-1 border border-slate-700 bg-slate-800 text-slate-300 rounded text-[9px] font-black hover:bg-slate-700 hover:text-white flex items-center gap-1 transition-colors uppercase cursor-pointer"
            >
              <RotateCw size={9} className={loading ? "animate-spin" : ""} /> REFRESH
            </button>
          </div>
...
function ContextSettingView({ user, isReadOnly, riskType }: { user: any, isReadOnly?: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  const fetchContextData = useCallback(async () => {
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        if (!isReadOnly) {
          try {
            ...
            console.error("Failed to write initial context:", writeErr);
          }
        }
        setFormData(initial);
      ...
      console.warn("Error fetching context, using initial state:", error);
      setFormData(initial);
      localStorage.setItem(cacheKey, JSON.stringify(initial));
      try {
        handleFirestoreError(error, OperationType.GET, `risk_context/${storageKey}`);
      } catch (e) {
        console.error("Firestore error handled during fetching context:", e);
      }
    } finally {
      setLoading(false);
    }
  ...
  const updateData = async (updates: any) => {
    try {
      // Optimistic update which immediately persists to local storage cache
...
function MonitoringCommunicationView({ user, isReadOnly, riskType }: { user: any, isReadOnly?: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  // Fetch Risk Identification Data non-realtime (same logic as Menu 5)
...
function MonitoringPlanPIView({ user, isReadOnly, riskType }: { user: any, isReadOnly?: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  // Fetch plan monitoring data non-realtime
...
function RiskOccurrenceMonitoringView({ user, isReadOnly, riskType }: { user: any, isReadOnly?: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  // Fetch Occurrence Monitoring data non-realtime
  const fetchOccurrenceData = useCallback(async () => {
    if (rows.length === 0) setLoading(true);
    try {
      const baseQuery = collection(db, 'risk_identification');
      const q = query(baseQuery, 
...
function FinalDocumentView({ user, isAdmin, isOperator, riskType }: { user: any, isAdmin: boolean, isOperator: boolean, riskType: 'strategis' | 'operasional' }) {
  ...
  const [masterLink, setMasterLink] = useState(() => {
    return localStorage.getItem('cached_master_link') || '';
  });
  const [opdLinks, setOpdLinks] = useState<Record<string, { driveLink: string; uploadLink: string }>>(() => {
    try {
      const cached = localStorage.getItem('cached_opd_links');
      return cached ? JSON.parse(cached) : {};
    } catch (e) {
      return {};
    }
  ...
  const fetchFinalDocuments = useCallback(async () => {
    const hasData = isSuper ? allUsers.length > 0 : finalDocs.length > 0;
    if (!hasData) setLoading(true);
    try {
      // 1. Fetch Master Storage Link (Global Fallback)
      const configSnap = await getDoc(doc(db, 'system_config', 'final_doc_storage'));
      if (configSnap.exists()) {
        const lnk = configSnap.data().link || '';
        setMasterLink(lnk);
        localStorage.setItem('cached_master_link', lnk);
      }

      // 2. Fetch Per-OPD Storage Links
      const opdSnap = await getDocs(collection(db, 'system_config', 'storage_links', 'opds'));
      const mapping: Record<string, { driveLink: string; uploadLink: string }> = {};
      opdSnap.forEach(d => {
        const data = d.data();
        mapping[d.id] = {
          driveLink: data.driveLink || data.link || '',
          uploadLink: data.uploadLink || ''
        };
      });
      setOpdLinks(mapping);
      localStorage.setItem('cached_opd_links', JSON.stringify(mapping));

      // 3. Fetch accounts & documents
      if (isSuper) {
        // Fetch all users (OPDs) to facilitate link mapping
        const usersSnap = await getDocs(collection(db, 'accounts'));
      ...
      const isOffline = err instanceof Error && (err.message.includes('offline') || err.message.includes('Internet') || err.message.includes('failed-precondition'));
      if (isOffline) {
        console.warn('FinalDocs super/user info (offline cached fallback active):', err);
      } else {
        console.error('FinalDocs super/user error:', err);
      }
    } finally {
      setLoading(false);
    }
  }, [user.uid, isSuper, allUsers.length, finalDocs.length]);
